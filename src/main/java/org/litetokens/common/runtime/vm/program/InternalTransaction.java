...
public class InternalTransaction {
   ...
   */
  public InternalTransaction(Transaction xlt,  InternalTransaction.XltType xltType)
      throws ContractValidateException {
    this.transaction = xlt;
    TransactionCapsule xltCap = new TransactionCapsule(xlt);
    this.protoEncoded = xltCap.getData();
    this.nonce = 0;
    ...
    this.deep = -1;
    if (xltType == XltType.XLT_CONTRACT_CREATION_TYPE) {
      ...
      this.transferToAddress = Wallet.generateContractAddress(xlt);
      this.note = "create";
    ...
    } else if (xltType == XltType.XLT_CONTRACT_CALL_TYPE) {
      TriggerSmartContract contract = ContractCapsule.getTriggerContractFromTransaction(xlt);
      if (contract == null) {
    ...
    this.hash = xltCap.getTransactionId().getBytes();
  }
  ...
  public InternalTransaction(byte[] parentHash, int deep, int index,
    ...
    // in this case, value also can represent a tokenValue when tokenId is not null, otherwise it is a xlt callvalue.
    this.value = value;
    this.data = ArrayUtils.nullToEmpty(data);
    this.nonce = nonce;
    this.hash = getHash();
    // in a contract call contract case, only one value should be used. xlt or a token. can't be both. We should avoid using
