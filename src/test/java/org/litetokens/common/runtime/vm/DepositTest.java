...
/*  pragma solidity ^0.4.0;
  ...
  public void loopCallTest()
    ...
    Transaction aXlt = LVMTestUtils.generateDeploySmartContractAndGetTransaction(
        contractA, address, aABI, aCode, value, fee, consumeUserResourcePercent, null, engeryLiimt);
    Runtime runtime = LVMTestUtils.processTransactionAndReturnRuntime(aXlt, DepositImpl.createRoot(manager), null);
    Assert.assertNull(runtime.getRuntimeError());

    Transaction bXlt = LVMTestUtils.generateDeploySmartContractAndGetTransaction(
        contractB, address, bABI, bCode, value, fee, consumeUserResourcePercent, null, engeryLiimt);
    runtime = LVMTestUtils.processTransactionAndReturnRuntime(bXlt, DepositImpl.createRoot(manager), null);
    Assert.assertNull(runtime.getRuntimeError());

    byte[] aAddress = Wallet.generateContractAddress(aXlt);
    byte[] bAddress = Wallet.generateContractAddress(bXlt);
    ...
    byte[] triggerData = LVMTestUtils.parseABI("callBcallARevert(address,uint256,uint256)", params1);
  ...
  public void loopCallTestOldVersion()
    ...
    Runtime runtime = LVMTestUtils.processTransactionAndReturnRuntime(aXlt, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());

    Transaction bXlt = LVMTestUtils.generateDeploySmartContractAndGetTransaction(
        contractB, address, bABI, bCode, value, fee, consumeUserResourcePercent, null);
    rootDeposit = DepositImpl.createRoot(manager);
    runtime = LVMTestUtils.processTransactionAndReturnRuntime(bXlt, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
