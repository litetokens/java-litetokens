...
import org.litetokens.core.config.Parameter.AdaptiveResourceLimitConstants;
...
import org.litetokens.core.db.api.AssetUpdateHelper;
...
public class Manager {
  ...
  public void pushVerifiedBlock(BlockCapsule block) throws ContractValidateException,
    ...
    pushBlock(block);
    logger.info("push block cost:{}ms, blockNum:{}, blockHash:{}, xlt count:{}",
  ...
  public boolean processTransaction(final TransactionCapsule xltCap, BlockCapsule blockCap)
    ...
    VMConfig.initAllowLvmTransferTrc10(dynamicPropertiesStore.getAllowLvmTransferTrc10());
    trace.init(blockCap);
  ...
  public synchronized BlockCapsule generateBlock(
    ...
    while (iterator.hasNext() || repushTransactions.size() > 0) {
      ...
      TransactionCapsule xlt;
      if (iterator.hasNext()) {
        fromPending = true;
        xlt = (TransactionCapsule) iterator.next();
      } else {
        xlt = repushTransactions.poll();
      }
  ...
  public void closeOneStore(ILitetokensChainBase database) {
