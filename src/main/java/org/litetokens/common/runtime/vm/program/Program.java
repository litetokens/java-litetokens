...
import static org.litetokens.common.runtime.utils.MUtil.transferAllToken;
...
import org.litetokens.core.actuator.TransferAssetActuator;
...
public class Program {
  ...
  /**
   * @param transferAddress the address send xlt to.
   * @param value the xlt value transferred in the internaltransaction
   */
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = convertToLitetokensAddress(getContractAddress().getLast20Bytes());
  ...
  public void createContract(DataWord value, DataWord memStart, DataWord memSize) {
    ...
    byte[] senderAddress = convertToLitetokensAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(convertToLitetokensAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] senderAddress = convertToLitetokensAddress(getContractAddress().getLast20Bytes());
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(convertToLitetokensAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public DataWord storageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(convertToLitetokensAddress(getContractAddress().getLast20Bytes()), key.clone());
    return ret == null ? null : ret.clone();
  }

  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    long ret = getContractState().getTokenBalance(convertToLitetokensAddress(address.getLast20Bytes()), String.valueOf(tokenId.longValue()).getBytes());
