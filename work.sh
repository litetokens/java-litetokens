...
# Function: to start, stop and restart java-litetokens.
...
JAR_FILE_PATH="./build/libs/java-litetokens.jar"
PID_FILE_PATH="java-litetokens.pid"
LOG_FILE_PATH="java-litetokens.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-litetokens."
        else
            echo "Failed to start java-litetokens."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-litetokens."
        else
            echo "Failed to stop java-litetokens."
        fi
    ;;
    restart)
