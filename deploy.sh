...
#)
stest_server=""
docker_num_in_67=`ssh -p 22008 -t java-litetokens@47.94.231.67 'docker ps -a | wc -l'`
docker_num_in_67=`echo $docker_num_in_67 | tr -d "\r"`
docker_num_in_122=`ssh -p 22008 -t java-litetokens@47.94.10.122 'docker ps -a | wc -l'`
...
if [[ "$TRAVIS_BRANCH" = "develop" || "$TRAVIS_BRANCH" = "master" || "$TRAVIS_BRANCH" = "Odyssey_v3.2.1" ]];then
  ...
  echo "init env"
  ssh java-litetokens@$stest_server -p 22008 $change_branch_CMD
  ssh java-litetokens@$stest_server -p 22008 sh /data/workspace/docker_workspace/do_stest.sh >$stestlogname 2>&1
