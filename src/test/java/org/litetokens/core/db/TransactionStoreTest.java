...
public class TransactionStoreTest {
  ...
  private static final String URL = "https://litetokens.co";
  ...
  public void GetTransactionTest() throws BadItemException, ItemNotFoundException {
    ...
    final TransactionStore xltStore = dbManager.getTransactionStore();
    ...
    xlt.setBlockNum(blockCapsule.getNum());
    blockStore.put(blockCapsule.getBlockId().getBytes(), blockCapsule);
    xltStore.put(xlt.getTransactionId().getBytes(), xlt);
    Assert.assertEquals("Get transaction is error",
        xltStore.get(xlt.getTransactionId().getBytes()).getInstance(), xlt.getInstance());
    ...
    tc =
      TransferContract.newBuilder()
        ...
        .setToAddress(ByteString.copyFromUtf8("bbb"))
        .build();
    xlt = new TransactionCapsule(tc, ContractType.TransferContract);
    Assert.assertNull(xltStore.get(xlt.getTransactionId().getBytes()));
