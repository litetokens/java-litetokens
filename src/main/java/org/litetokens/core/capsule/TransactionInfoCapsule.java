  ...
  public static TransactionInfoCapsule buildInstance(TransactionCapsule xltCap, BlockCapsule block,
    ...
    if (Args.getInstance().isSaveInternalTx() && null != programResult.getInternalTransactions()) {
      for (InternalTransaction internalTransaction : programResult
        ...
        Protocol.InternalTransaction.Builder internalXltBuilder = Protocol.InternalTransaction
            .newBuilder();
        // set hash
        internalXltBuilder.setHash(ByteString.copyFrom(internalTransaction.getHash()));
        // set caller
        internalXltBuilder.setCallerAddress(ByteString.copyFrom(internalTransaction.getSender()));
        // set TransferTo
        internalXltBuilder
        ...
        internalTransaction.getTokenInfo().forEach((tokenId, amount) -> {
          ...
          internalXltBuilder.addCallValueInfo(tokenInfoBuilder);
        });
        // Token for loop end here
        internalXltBuilder.setNote(ByteString.copyFrom(internalTransaction.getNote().getBytes()));
        internalXltBuilder.setRejected(internalTransaction.isRejected());
        builder.addInternalTransactions(internalXltBuilder);
      }
    }
